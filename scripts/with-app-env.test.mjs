...
test("resolves the local vite CLI to this Node process plus vite.js", () => {
  const r = resolveSpawn("vite", ["--version"]);
  assert.equal(r.command, process.execPath);
  assert.equal(r.shell, false);
  assert.match(r.args[0].replaceAll("\\", "/"), /\/vite\/bin\/vite\.js$/);
  assert.deepEqual(r.args.slice(1), ["--version"]);
});

test("resolves the local tauri CLI to this Node process plus tauri.js", () => {
  const r = resolveSpawn("tauri", ["dev"]);
  assert.equal(r.command, process.execPath);
  assert.equal(r.shell, false);
  assert.match(r.args[0].replaceAll("\\", "/"), /\/@tauri-apps\/cli\/tauri\.js$/);
  assert.deepEqual(r.args.slice(1), ["dev"]);
});

test("packageCli reads vite's bin field", () => {
  const p = packageCli("vite", "vite");
  assert.ok(p);
  assert.match(p.replaceAll("\\", "/"), /\/vite\/bin\/vite\.js$/);
});

test("leaves a direct node invocation untouched", () => {
  const r = resolveSpawn(process.execPath, ["-e", "0"]);
  assert.equal(r.command, process.execPath);
  assert.equal(r.shell, false);
  assert.deepEqual(r.args, ["-e", "0"]);
});

test("the wrapper actually launches vite --version (no spawn ENOENT)", async () => {
  const { stdout, stderr } = await execFileAsync(process.execPath, [WRAPPER, "vite", "--version"]);
  const text = `${stdout}${stderr}`;
  assert.match(text, /vite\//i);
});
